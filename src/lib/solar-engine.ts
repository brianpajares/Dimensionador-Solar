...
import type { Appliance, OffGridResult, GridTiedResult, FinancialResult, CatalogItem, BomItem, SystemType } from '../types';
import { CATALOG, CATALOG_VERSION } from './catalog.js';

export const ENGINE_VERSION = 'solar-engine-2026.08.10';

export const ETA_INVERTER = 0.90;
export const ETA_BATTERY = 0.85;
export const ETA_PV_PATH = 0.75;
export const OVERSIZE_PV = 1.15;
export const CTRL_MARGIN = 1.25;
export const INV_SURGE = 1.25;
...
export function sizeOffGrid(input: {
  ...
  const E_daily = E_ac / ETA_INVERTER;
  ...
  return { E_daily, E_ac, battWh, battAh, arrayWp, controllerA, inverterW, DoD, systemVoltage };
}
...
export function sizeGridTied(input: {
  ...
  const inverterKw = kWp / 1.15;
  return { kWp, nPanels, inverterKw };
}
...
export function calculateIRR(cashFlows: number[]): number {
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    if (Math.abs(npv) < precision) return mid * 100;
    if (npv > 0) low = mid;
    else high = mid;
  }
  ...
  if (params.systemType === 'offgrid' && params.offgridResult) {
    ...
    if (chemistry === 'lifepo4') {
      if (res.systemVoltage === 12) {
        ...
        nBatteries = Math.ceil(res.battWh / (selectedBattery.capacity_wh || 1280));
      } else if (res.systemVoltage === 24) {
        selectedBattery = CATALOG.find(item => item.id === 'battery-pylontech-24v-100ah')!;
        nBatteries = Math.ceil(res.battWh / (selectedBattery.capacity_wh || 2560));
      } else {
        selectedBattery = CATALOG.find(item => item.id === 'battery-pylontech-48v-100ah')!;
        nBatteries = Math.ceil(res.battWh / (selectedBattery.capacity_wh || 4800));
      }
    } else {
    ...
    if (res.systemVoltage === 12) inverter = CATALOG.find(item => item.id === 'inverter-offgrid-must-1kw')!;
    else if (res.systemVoltage === 24) inverter = CATALOG.find(item => item.id === 'inverter-offgrid-must-3kw')!;
    else inverter = CATALOG.find(item => item.id === 'inverter-offgrid-must-5kw')!;

    if (res.inverterW > (inverter.power_w || 1000)) {
      if (inverter.id === 'inverter-offgrid-must-1kw') inverter = CATALOG.find(item => item.id === 'inverter-offgrid-must-3kw')!;
      else if (inverter.id === 'inverter-offgrid-must-3kw') inverter = CATALOG.find(item => item.id === 'inverter-offgrid-must-5kw')!;
    }
    ...
    if (invWNeeded > 2000 && invWNeeded <= 3600) selectedInv = CATALOG.find(item => item.id === 'inverter-grid-growatt-3.6kw')!;
    else if (invWNeeded > 3600 && invWNeeded <= 5000) selectedInv = CATALOG.find(item => item.id === 'inverter-grid-growatt-5kw')!;
    else if (invWNeeded > 5000) selectedInv = CATALOG.find(item => item.id === 'inverter-grid-growatt-10kw')!;
